...
func (p *Post) formatContent(cfg *config.Config, c *Collection, isOwner bool, isPostPage bool) {
	...
	if exc := strings.Index(p.Content, "<!--more-->"); exc > -1 {
...
func applyCommonmarkSpecial(data []byte, skipNoFollow bool, baseURL string, cfg *config.Config) string {
	...
	if baseURL != "" {
		...
		htm = mentionReg.ReplaceAll(htm, []byte("<a href=\""+handlePrefix+"$1$2\" class=\"u-url mention\">@<span>$1$2</span></a>"))
	}
	// Strip out bad HTML
...
func applySaturdaySpecial(data []byte, skipNoFollow bool, baseURL string, cfg *config.Config) string {
	...
	// Generate Markdown
	md := blackfriday.Markdown(data, blackfriday.HtmlRenderer(htmlFlags, "", ""), mdExtensions)
	if baseURL != "" {
		...
		md = hashtagReg.ReplaceAll(md, []byte("<a href=\""+tagPrefix+"$1\" class=\"hashtag\"><span>#</span><span class=\"p-category\">$1</span></a>"))
		handlePrefix := cfg.App.Host + "/@/"
		md = mentionReg.ReplaceAll(md, []byte("<a href=\""+handlePrefix+"$1$2\" class=\"u-url mention\">@<span>$1$2</span></a>"))
	}
	// Strip out bad HTML
	policy := getSanitizationPolicy()
	policy.RequireNoFollowOnLinks(!skipNoFollow)
...
func handleRenderMarkdown(app *App, w http.ResponseWriter, r *http.Request) error {
	...
	out := struct {
		...
		Body: applyMarkdown([]byte(in.RawBody), in.CollectionURL, app.Config()),
	}
