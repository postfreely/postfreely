...
func InitRoutes(apper Apper, r *mux.Router) *mux.Router {
	...
	wf := webfinger.Default(wfResolver{apper.App().db, apper.App().Config()})
	wf.NoTLSHandler = nil
	...
	ni := nodeinfo.NewService(*niCfg, nodeInfoResolver{apper.App().Config(), apper.App().db})
	...
	if apper.App().Config().App.OpenRegistration {
